...
Examples:
  ...
  python3 s3_cleanup.py --dry-run             # Test mode - no actual deletions
...
Features:
  ...
  - Improved error handling and permission checking
  - Dry-run mode for testing
    ...
    parser.add_argument(
        '--dry-run', '-d',
        action='store_true',
        help='Dry run mode - analyze and show what would be deleted without actually deleting anything'
    )
    ...
    try:
        ...
        print(f"{Colors.YELLOW}For debugging, try running with --dry-run first{Colors.END}")
        sys.exit(1)
